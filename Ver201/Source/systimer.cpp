...
/*
  //Не используется из-за увеличенного уровня помех на АЦП
  //Тактирование от HSE с PLL:
  RCC->CR |=  RCC_CR_HSEON;               //включение генератора HSE (8 MHz)
  ...
  while((RCC->CR & RCC_CR_PLLRDY) == 0);  //ожидание готовности PLL
  RCC->CFGR |= RCC_CFGR_SW_PLL;           //выбор тактирования от PLL
  while((RCC->CFGR & RCC_CFGR_SWS) != RCC_CFGR_SWS_PLL); //ожидание выбора PLL
  RCC->CR &= ~RCC_CR_HSION;               //выключение HSI
*/
